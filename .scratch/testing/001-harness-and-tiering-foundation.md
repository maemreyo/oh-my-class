...
- [x] `real_llm` marker exists; `uv run pytest -m "not real_llm"` runs the fast tier (real DB, no LLM) and `-m real_llm` runs the eval tier.
- [x] A real-LLM test fixture targets 9router `:20228` / `4omc`; no test reaches OpenAI or any default provider.
- [x] Real test-DB fixtures apply migrations and isolate per test; no DB mocks.
- [x] DeepEval judge is 9router-backed and offline (no telemetry/egress); results are logged to Langfuse.
- [x] A lint/test fails the build if `FakeListLLM`/`GenericFakeChatModel` (or equivalent LLM fakes) are imported anywhere.
- [x] Docs state the policy + how to run each tier.

## Detailed test suite

- [x] `tests/test_harness_tiering.py`: fast-tier collection contains zero `real_llm` tests; eval-tier collection is non-empty.
- [x] `tests/test_harness_llm_routing.py`: the real-LLM fixture issues a live call to 9router (`4omc`) and returns; no fallback to OpenAI.
- [x] `tests/test_no_fake_llm.py`: importing a fake LLM anywhere fails the lint check.
- [x] DB fixture test: a migrated ephemeral DB round-trips a run row and rolls back between tests.
- [x] Run `uv run pytest -m "not real_llm" -q` and `uv run pytest -m real_llm tests/test_harness_llm_routing.py -q`.
