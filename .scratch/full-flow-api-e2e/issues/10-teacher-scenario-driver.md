...
- [x] `scripts/run_teacher_scenarios.py` (httpx) driving pure REST:
...
- [x] Cover the FULL output matrix (ADR-031): request ALL 9 artifact types (lesson, worksheet,
...
- [x] Output per scenario to `.scratch/teacher-scenarios/<NN-scenario>/`:
...
- [x] Modes: real-LLM (default, per real-not-mock policy, 9router :20228 / model 4omc) and an
      optional fixture-LLM mode for fast CI.
- [x] Prereqs auto-check: Postgres up, gateway up (worker in-process), exporter CLI built;
