...
- [ ] `GateInterrupt.context` for `content_approval` includes a `quality_report` field (type `ArtifactQualityReport | None`).
- [ ] The frontend `content_approval` body renders per-layer quality indicators when `quality_report` is present (gracefully absent-safe — shows nothing if `None`).
- [ ] A teacher can see at a glance: overall score, which layers flagged issues, and the top-level reason for each flag.
- [ ] No change to approval mechanics — the quality report is informational only; the teacher still decides.
- [ ] Schema parity: `quality_report` field added to the Zod-codegen'd type; FE uses the generated type.

## Detailed test suite

(Deterministic-logic tests only — no real LLM needed for this issue.)

- [ ] `services/gateway/tests/test_content_approval_gate_context.py`: when `content_approval` interrupt fires, `gate_interrupt.context["quality_report"]` contains the expected `ArtifactQualityReport` shape (per-layer scores present, pass boolean, overall score).
- [ ] FE unit (Vitest): `content-approval-body.test.tsx` — snapshot test that per-layer flags render; test that absent `quality_report` renders nothing extra (no crash).
- [ ] Schema parity test (`scripts/verify_schema_parity.py`): `ArtifactQualityReport` Zod schema is generated and matches the Pydantic contract.
