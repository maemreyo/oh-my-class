...
| **`vocabulary-batch/` (12)** | ✅ all | ⚠️ **PARTIAL (corrected 2026-07-08 — was ❌ POTEMKIN)** — re-read of `vocabulary_batch_orchestrator.py` during the `.scratch/design-reflection-2026-07-08.md` grill session found `_process_cluster` now DOES fully chain grounding→synthesis→practice→gate internally (`run_vocabulary_batch_orchestrator` is called from `teaching_pack/nodes.py:453`, no longer stops at `queued`). The remaining real gap: `FEATURE_VOCABULARY_BATCH_V1` defaults `false`, and **no route/API/UI anywhere ever sets `contract.mode="vocabulary_batch"`** — the feature has no entry point, not a broken internal pipeline. Tracked as `LIC-08` (`.scratch/llm-integration-completion/`). |
...
> ⚠️ **AUDIT (corrected 2026-07-08): PARTIAL, not POTEMKIN** — the orchestrator's internal chain (grounding→synthesis→practice→gate) is real and complete as of the current `vocabulary_batch_orchestrator.py`. The actual gap is that `FEATURE_VOCABULARY_BATCH_V1` defaults off and nothing in production ever sets `contract.mode="vocabulary_batch"` — no entry point exists yet. See `LIC-08` in `.scratch/llm-integration-completion/`.
