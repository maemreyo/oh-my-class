...
| **`vocabulary-batch/` (12)** | ✅ all | ✅ **REAL (corrected 2026-07-08, twice — was ❌ POTEMKIN, briefly ⚠️ PARTIAL)** — `_process_cluster` fully chains grounding→synthesis→practice→gate. `LIC-08` (2026-07-08) found the "no route" theory was **wrong**: `contract.mode="vocabulary_batch"` already worked via the existing freeform `class_info.mode` passthrough (`run_contract_setup.py`'s `_mode()`) — no schema change needed. The real gap was narrower: `normalize_vocabulary_input` (a real, tested `InputNormalizationReport` builder) had zero callers, so `input_normalization_report` was never populated before the orchestrator read it. Now wired into `_artifact_workflow`. Only remaining item: `FEATURE_VOCABULARY_BATCH_V1` still defaults `false` (a deliberate rollout-flag decision, left to whoever owns feature rollout — not a bug). |
...
| **048** | `docs/adr/048-planner-blueprint-deterministic-by-design.md` | Planner lesson-blueprint generation stays deterministic (Gagné/UbD staged engine); real LLM branch kept as a reachable fallback keyed on seed/template coverage, not a dead literal |
| **049** | `docs/adr/049-slide-deck-remains-deterministic-content-creator-flips.md` | `content_creator`'s 9 non-slide-deck artifact types move section content to real LLM; `slide_deck` explicitly excluded — its real-content path is ADR-047's `ContentMaterializer`, tracked independently |
| **050** | `docs/adr/050-unit-planner-sequence-critic-deterministic-deferred.md` | `unit_planner`/`sequence_critic` stay deterministic pending `td-006`/`td-021`; misleading "Curricular-CoT adapt" `rationale` string corrected immediately, independent of that backlog |
...
> ✅ **AUDIT (corrected 2026-07-08): REAL** — the orchestrator's internal chain (grounding→synthesis→practice→gate) is real and complete, and as of `LIC-08` the missing `normalize_vocabulary_input` wiring (the actual gap — not a missing route, see audit table) is fixed too. Only `FEATURE_VOCABULARY_BATCH_V1`'s off-by-default rollout flag remains, a deliberate ops decision, not a code gap.
...
## LLM Integration Completion & Governance track (added 2026-07-08, from the 100-question `.scratch/design-reflection-2026-07-08.md` grill session)

> **Implementation status (2026-07-08): all 9 `LIC-*` + all 8 `LGH-*` issues done, except `LGH-06` (Phase 1/4 done, Phases 2-4 remain — see that issue).** Two new issues filed from real bugs found while implementing: `LIC-10` (`_class_profile` silently discards real `class_info` when `persona_snapshot={}` — affects the already-shipped staged planner, not just this session's work) and `.scratch/content-safety/CS-02-...` (2 promptfoo scenarios fail a genuine 3-repeat majority vote — real security gaps, not test flakiness). Also corrected mid-implementation: `LIC-06`'s `concept_alignment` half turned out to have no real integration point (left `KNOWN_DARK` instead of force-wired); `LIC-07`'s `roadmap_agent` has zero product surface expecting it (left `KNOWN_DARK`); `LIC-08`'s "missing route" premise was wrong — the real gap was an unwired `normalize_vocabulary_input`, not routing. Full audit trail is in each issue file's own done-note.

17 issues across 2 sub-tracks, each `status: ready-for-agent`/`ready`, produced by auditing which sub-agents in the teaching-pack pipeline actually call an LLM in production vs. run real-tested-but-flag-shadowed or hard-coded-flag-shadowed deterministic code (the same "green but hollow" shape as the 2026-07-01 audit above, one level deeper: the function IS called, but a flag one line above the call site picks the deterministic branch).

- **`llm-integration-completion/` (LIC-01..09)** — product-impact fixes. `LIC-01` wire `AdaptiveJudge` as the real reviewer quality gate (do first) · `LIC-02` flip `content_creator`'s 9 non-slide-deck artifact types to real LLM content (ADR-049, after LIC-01) · `LIC-03` diagnostician: keep deterministic error-rate/severity math, flip misconception classification + summary to LLM · `LIC-04` planner: replace hard-coded `use_staged_planner=True` with a real seed-coverage condition (ADR-048) · `LIC-05` fix unit_planner's misleading "Curricular-CoT adapt" `rationale` string (ADR-050, independent/immediate) · `LIC-06` wire `concept_alignment`+`coherence_judge` (real, tested, zero-caller) into practice QA and unit_planner validation · `LIC-07` investigate/wire `roadmap_agent` (distinct feature from `content_creator`'s `roadmap` artifact type — needs product confirmation first) · `LIC-08` build the missing route/flag for `vocabulary_batch` mode — **corrects the audit table's vocabulary-batch verdict above from POTEMKIN to PARTIAL** · `LIC-09` promptfoo N-sample majority vote (reuse `packages.quality.layer4_judge.majority_vote`) + unskip `test_deepeval_real_llm.py` (stale skip reason — its blocker was already fixed in `ec10283`).
- **`llm-governance-hardening/` (LGH-01..08)** — process/infra fixes so this pattern surfaces on its own next time instead of waiting for another manual audit. `LGH-01` import-linter contract banning `openai`/`litellm` outside `packages.llm_client` (+ delete dead `complete_non_streaming_chat`) · `LGH-02` grep-based CI check for raw LLM-gateway host/port outside the 2 legitimate health-probe files · `LGH-03` config-drift lint (env vars used in code vs. documented in `.env.example`) · `LGH-04` formalize `.scratch` issue `status` schema + non-blocking stale-issue report · `LGH-05` document the safe-by-default pluggable-transport principle + lightweight warning lint · `LGH-06` phased, fully-tested migration of 16 direct `os.environ` readers to `pydantic-settings` (+ ban `${VAR}` in `.env*` now) · `LGH-07` dark-code standing report (auto-surface new zero-caller symbols against the existing `REQUIRE_WIRED`/`KNOWN_DARK` ledger) · `LGH-08` `# BLOCKED-ON` marker convention + listing script, applied now to `ownership.py`'s `SCHOOL_ADMIN`/`organization_id` gap (tracked as its own issue, `.scratch/multi-tenancy/organization-id-migration.md` — needs data-model scoping, not rushed here; also the same gap ADR-047 Decision #9 already flagged as a soft dependency of the slide-deck editor's authorization).

**Sequencing (agreed during the grill session):** (1) `LIC-01` → `LIC-02` (product-impact, quality gate before content variance increases) → (2) independent quick items in parallel: `LIC-03`, `LIC-04`, `LIC-05`, `LIC-06`, `LGH-01`, `LGH-02`, `LGH-06`'s `${VAR}` fix, `LIC-09`, `LGH-08`'s marker → (3) documentation/reporting: ADRs 048-050 (already written), `LGH-04`, `LGH-05`, `LGH-07` → (4) `LGH-06`'s full phased env-var migration (largest, highest-risk, done last). `LIC-07` and the `organization_id` migration need product/data-model scoping before they can start.

> No shared dependency found between this track and `td-*`/`vb-*`/`el-*`/Slide Deck — runs fully in parallel, except `LIC-08` corrects a `vb-*` verdict above (no code dependency, just documentation) and `LGH-08`'s tracked migration is a soft dependency of ADR-047 Decision #9.
...
**`vocabulary-batch/`** placement: `vb-001` W0 → `vb-002,003` W1 → `vb-004,006` W2 → `vb-005` W3 → `vb-007,008` W4 → `vb-009,011` W5 → `vb-010` W6 → `vb-012` W7. All marked done per-issue; corrected 2026-07-08 to ⚠️ PARTIAL feature-level (internal chain is real — see audit table) — the open gap is a missing entry point/route, not the pipeline. Tracked as `LIC-08`.
