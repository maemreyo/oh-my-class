...
- [ ] The coherence pass runs lazily (on `complete` or on demand), never as a blocking gate.
- [ ] It produces structured `CoherenceWarning[]` with type + involved sessions + message.
- [ ] Warnings surface on the dashboard via the unit event stream; a unit can still be `complete`/exported with open warnings.
- [ ] Detects: terminology drift, non-monotonic difficulty, redundant coverage, unresolved back-references.

## Detailed test suite

(Real LLM via 9router port 20228, model `4omc`, for the semantic checks; deterministic for back-reference resolution.)

- [ ] `packages/agents/tests/test_unit_coherence.py`: a sequence where session 4 calls a concept "tỉ số" that session 2 called "phân số" yields a terminology warning naming both sessions.
- [ ] same file: a sequence whose difficulty dips mid-way yields a monotonic-difficulty warning; a clean sequence yields none.
- [ ] same file: an unresolved "as learned in session 9" reference (no such session) yields a back-reference warning (deterministic).
- [ ] Non-blocking test: a unit with open coherence warnings is still `complete` and exportable.
- [ ] Run `uv run pytest packages/agents/tests/test_unit_coherence.py -v`.
