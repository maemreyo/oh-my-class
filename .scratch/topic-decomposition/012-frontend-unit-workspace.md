...
- [x] `/units/[parentRunId]` renders session cards with live status, unit progress, and unit/per-session actions.
- [x] `useUnit` hydrates from the read model and applies live deltas with cursor reconciliation; counts come from the backend, not derived client-side.
- [x] The `UNIT_APPROVAL` body supports reorder/add/remove/edit and renders the prerequisite DAG and theme preview; reorder preserves `session_id` references.
- [x] A run becoming a unit redirects to the workspace; contract confirmation precedes redirect in the linear shell.
- [x] Per-session review reuses the `CONTENT_APPROVAL` body; no duplicate review UI.
- [x] All unit types are imported from generated schemas; no hand-written transport interfaces.

## Detailed test suite

- [x] `apps/web/tests/use-unit.test.ts` (vitest): hydrate + apply a `unit.session.status_changed` delta updates the right card; a stale (`cursor` ≤ snapshot) delta is ignored; reconnect re-snapshots.
...
- [x] `apps/web/tests/unit-dashboard.test.tsx` (RTL): `Approve all` calls approve-all; `spawn-anyway` calls the unblock endpoint; a blocked card shows the override affordance.
...
- [x] Run `pnpm -F web test` and `pnpm -F web test:e2e`.
