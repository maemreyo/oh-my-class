...
const questions = [
  {
    n: 1, cat: "cat1", text: "The plane ___ before we arrived at the airport, so we missed our flight.",
    options: ["A. left", "B. had left", "C. has left", "D. was leaving"],
    correct: 1,
    essence: "Hành động 'left' xảy ra TRƯỚC một mốc quá khứ khác ('we arrived') → quá khứ hoàn thành.",
    ...
    wrong: [
      ...
      { l: "C", opt: "has left", why: "Hiện tại hoàn thành không hợp với mốc thời gian quá khứ 'arrived'." },
      { l: "D", opt: "was leaving", why: "Diễn tả hành động đang xảy ra, không phải đã hoàn tất trước đó." },
    ],
  },
  {
    n: 2, cat: "cat2", text: "A: \"Could you tell me how much the ___ to the airport is?\" B: \"About 200,000 dong by taxi.\"",
    options: ["A. fare", "B. ticket", "C. fee", "D. cost"],
    correct: 0,
    essence: "Hỏi giá cho một chuyến di chuyển bằng phương tiện (taxi) → Fare, không phải Fee (dịch vụ/tổ chức).",
    trap: "C 'fee' hay bị chọn nhầm vì cũng là 'tiền phải trả', nhưng fee không gắn với phương tiện di chuyển.",
    wrong: [
      { l: "B", opt: "ticket", why: "Ticket là vật thể, không dùng để hỏi 'how much' trực tiếp theo cách này." },
      { l: "C", opt: "fee", why: "Fee dùng cho dịch vụ/tổ chức, không phải tiền taxi." },
      { l: "D", opt: "cost", why: "Cost là từ chung chung hơn — câu này cần từ chuyên biệt trong cụm đã học." },
    ],
  },
  {
    n: 3, cat: "cat3", text: "Rewrite: \"They didn't compensate her for the damaged luggage.\" → \"She ___\"",
    options: ["A. wasn't compensated for the damaged luggage.", "B. didn't compensate for the damaged luggage.", "C. hasn't been compensating the damaged luggage.", "D. isn't compensated the damaged luggage."],
    correct: 0,
    ...
    wrong: [
      { l: "B", opt: "didn't compensate for...", why: "Vẫn ở dạng chủ động — thiếu 'was/were + V3'." },
      { l: "C", opt: "hasn't been compensating...", why: "Sai thì — đề bài ở quá khứ đơn, không phải hiện tại hoàn thành tiếp diễn." },
      { l: "D", opt: "isn't compensated...", why: "Sai thì — dùng hiện tại thay vì quá khứ." },
    ],
  },
  {
    n: 4, cat: "cat4", text: "Fill in: \"After a long ___ across the desert, the caravan finally reached the oasis, exhausted but relieved.\"",
    options: ["A. voyage", "B. journey", "C. trip", "D. travel"],
    correct: 1,
    essence: "Nhấn mạnh quá trình di chuyển dài, gian khổ, qua nhiều chặng ('across the desert') → Journey.",
    ...
    wrong: [
      { l: "A", opt: "voyage", why: "Chỉ dùng cho phương tiện lớn (tàu/phi thuyền), không hợp với 'caravan across the desert'." },
      { l: "C", opt: "trip", why: "Trip ngắn ngày, có mục đích rõ ràng — không hợp với 'long ... exhausted'." },
...
function qcard(q) {
  const opts = q.options
    .map(
      (o, i) => `<div class="art-option${i === q.correct ? " art-option--correct" : ""}"><span class="art-letter">${o[0]}</span><span>${o.slice(3)}</span></div>`
    )
    .join("\n");
  return `
  <div class="art-qcard art-g-${q.cat} art-avoid-break" id="q${q.n}">
...
<div class="art-shell--split">
  <aside class="art-shell-aside">
    ...
    <div class="art-jumpbox" style="margin-bottom:16px;">
      <input type="text" placeholder="Câu #" />
      <span style="font-size:12px;color:var(--art-ink-faint);font-family:var(--art-font-mono);">→ Enter</span>
    </div>
    <label class="art-mode-toggle" style="margin-bottom:18px;">
      <span class="art-switch art-on"></span> Hiện đáp án
    </label>
    <div class="art-side-legend" style="display:flex;flex-direction:column;gap:6px;">
      ${groupLegend
        .map(
          (g) => `<div style="display:flex;align-items:center;gap:8px;font-size:12px;color:var(--art-ink-soft);">
  ...
  <div class="art-shell-main">
    ...
    <div class="art-qgrid" style="margin-bottom:32px;max-width:260px;">
      ${questions.map((q) => `<button class="art-g-${q.cat}">${q.n}</button>`).join("")}
    </div>
...
writeFile("families/exam-key.html", renderPage({ family: "paper-dossier", title: "Answer Key — Unit 2 Quiz", body }));
