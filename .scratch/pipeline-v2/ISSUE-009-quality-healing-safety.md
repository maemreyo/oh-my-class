...
- Persisted recovery orchestration seam was closed on the active `/teaching-packs/*` surface: `services/gateway/teaching_pack_executor.py` now recognizes `quality_recovery_route` states returned by the graph, persists a `teaching_pack.quality_recovery.started` internal event, and records status-history stage/reason for the targeted recovery route instead of marking the run failed with `missing_export_evidence`.
- Adaptive judge/rubric governance was tightened in `packages/quality/layer4_judge`: `JudgePolicyContext` records deterministic policy triggers for high/rigorous risk, deterministic issues, and borderline scores; `RubricSelector` now composes rubric version IDs from artifact type, subject, locale, curriculum, risk, and deterministic failure context so judge provenance is audit-ready.
...
- Focused verification after the recovery-persistence update: `uv run pytest services/gateway/tests/test_teaching_pack_executor.py -q` → `10 passed`; `uv run basedpyright services/gateway/teaching_pack_executor.py services/gateway/tests/test_teaching_pack_executor.py` → `0 errors`; `uv run python -m py_compile services/gateway/teaching_pack_executor.py services/gateway/tests/test_teaching_pack_executor.py` → success; manual completion-recorder driver verified `quality_recovery_route=artifact_workflow` persists `teaching_pack.quality_recovery.started` with the route and issues payload.
- Focused verification after the adaptive judge policy update: `uv run pytest packages/quality/tests/test_judge_interface.py -q` → `37 passed`; `uv run basedpyright packages/quality/layer4_judge/judge_policy.py packages/quality/layer4_judge/rubric_selector.py packages/quality/layer4_judge/judge_interface.py packages/quality/tests/test_judge_interface.py` → `0 errors`; `uv run python -m py_compile packages/quality/layer4_judge/judge_policy.py packages/quality/layer4_judge/rubric_selector.py packages/quality/layer4_judge/judge_interface.py packages/quality/tests/test_judge_interface.py` → success; manual `AdaptiveJudge` driver verified contextual rubric `rubric-quiz-subject-math-locale-vi-vn-curriculum-ct-2018-risk-rigorous-answer_key_leakage` with policy reasons `risk:rigorous`, `deterministic_issues`, and `borderline_score`.
...
- Adaptive LLM judge/rubric governance now has deterministic trigger and contextual rubric provenance coverage. Live 9Router judge evidence remains broader Pipeline V2 release evidence.
- Pack-coherence failures are deterministic blocks before approval/export and now route through the active graph recovery seam. The active completion recorder persists those recovery states as resumable recovery events/status history. Full live 9Router evidence for every recovery branch remains broader Pipeline V2 work.
- Persisted repair loops for every listed failure class remain broader Pipeline V2 work. The focused slice now has deterministic hard blocks, explicit typed routes for factual uncertainty/pedagogical mismatch, scoped teacher regeneration proof, fail-closed timeout/malformed JSON evidence, and recovery-route persistence evidence.

## Active Surface Closure — 2026-06-29

Status: COMPLETE for the active `/teaching-packs/*` surface. Remaining live proof for every recovery branch is tracked by the release matrix, not by this issue.

Evidence:
- Fresh focused quality/healing gate: `uv run pytest services/gateway/tests/test_quality_gates.py services/gateway/tests/test_healing_executors.py common/contracts/tests/test_quality.py packages/agents/tests/teaching_pack/test_render_quality.py services/gateway/tests/test_teaching_pack_executor.py packages/quality/tests/test_judge_interface.py -q` → 64 passed.
- Strict typing gate: `uv run basedpyright services/gateway/quality_gates.py services/gateway/healing_executors.py common/contracts/quality.py packages/agents/teaching_pack/quality.py packages/agents/teaching_pack/quality_routing.py packages/agents/teaching_pack/nodes.py services/gateway/teaching_pack_executor.py packages/quality/layer4_judge/judge_policy.py packages/quality/layer4_judge/rubric_selector.py packages/quality/layer4_judge/judge_interface.py packages/agents/tests/teaching_pack/test_render_quality.py` → 0 errors, 0 warnings, 0 notes.
- Compile/diff gates: `uv run python -m py_compile ...` succeeded and `GIT_MASTER=1 git diff --check` was clean.
