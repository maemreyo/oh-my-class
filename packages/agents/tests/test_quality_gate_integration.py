...
class TestSchemaValidator:
    def test_passes_valid_artifacts(self):
        ...
        result = step_09_schema_validate(cast("OhMyClassState", _base_state(artifacts=[VALID_ARTIFACT])))
        assert result["schema_valid"] is True
        assert "fail_layer" not in result

    def test_fails_empty_artifacts(self):
        from packages.agents.gates.schema_validator import step_09_schema_validate
        result = step_09_schema_validate(cast("OhMyClassState", _base_state(artifacts=[])))
    ...
    def test_fails_missing_keys(self):
        from packages.agents.gates.schema_validator import step_09_schema_validate
        result = step_09_schema_validate(cast("OhMyClassState", _base_state(artifacts=[{"content": "test"}])))
        assert result["schema_valid"] is False
        assert result["fail_layer"] == "schema"

    def test_fails_empty_content(self):
        from packages.agents.gates.schema_validator import step_09_schema_validate
        result = step_09_schema_validate(cast("OhMyClassState", _base_state(artifacts=[{"type": "lesson", "content": ""}])))
        assert result["schema_valid"] is False

    def test_preserves_fail_count_from_state(self):
        from packages.agents.gates.schema_validator import step_09_schema_validate
        result = step_09_schema_validate(cast("OhMyClassState", _base_state(artifacts=[], fail_count=2)))
...
class TestContentReviewer:
    def test_passes_clean_content(self):
        ...
        result = step_10_content_review(cast("OhMyClassState", _base_state(artifacts=[VALID_ARTIFACT])))
        assert result["content_review_passed"] is True
        assert "fail_layer" not in result

    def test_fails_blocked_content(self):
        from packages.agents.gates.content_reviewer import step_10_content_review
        state = _base_state(artifacts=[{"type": "lesson", "content": "violence and gore in this lesson"}])
    ...
    def test_fails_html_with_external_assets(self):
        from packages.agents.gates.content_reviewer import step_10_content_review
        artifact = {"type": "lesson_html", "content": '<!DOCTYPE html><html><body><img src="https://ext.com/i.png"></body></html>'}
...
class TestLLMJudge:
    ...
    def test_empty_content_scores_zero(self):
        from packages.agents.gates.llm_judge import step_10b_llm_judge
        result = step_10b_llm_judge(cast("OhMyClassState", _base_state(artifacts=[{"type": "lesson", "content": ""}])))
        assert result["fail_layer"] == "judge"
...
class TestHealingOrchestrator:
    ...
    def test_escalate_sets_error_field(self):
        from packages.agents.healing.orchestrator import HealingOrchestrator
        state = cast("OhMyClassState", _base_state(fail_count=3, fail_type="score", fail_layer="judge"))
        result = HealingOrchestrator(max_retries=3).heal(state)
...
class TestRoutingAfterSchema:
    def test_routes_to_content_review_on_pass(self):
        ...
        assert route_after_schema(cast("OhMyClassState", {"schema_valid": True})) == "step_10_content_review"
...
class TestRoutingAfterContentReview:
    def test_routes_to_judge_on_pass(self):
        ...
        assert route_after_content_review(cast("OhMyClassState", {"content_review_passed": True})) == "step_10b_llm_judge"

    def test_routes_to_healing_on_fail(self):
        from packages.agents.graph import route_after_content_review
        assert route_after_content_review(cast("OhMyClassState", {"content_review_passed": False})) == "healing_node"
...
class TestRoutingAfterJudge:
    def test_routes_to_gate_on_high_score(self):
        ...
        assert route_after_judge(cast("OhMyClassState", {"judge_score": 7.0})) == "gate_02_content_approval"
        assert route_after_judge(cast("OhMyClassState", {"judge_score": 9.5})) == "gate_02_content_approval"

    def test_routes_to_healing_on_low_score(self):
        from packages.agents.graph import route_after_judge
        assert route_after_judge(cast("OhMyClassState", {"judge_score": 5.0})) == "healing_node"
...
class TestQualityGateChain:
    def test_valid_artifacts_pass_all_three_gates(self):
        ...
        from packages.agents.gates.content_reviewer import step_10_content_review
        from packages.agents.gates.llm_judge import step_10b_llm_judge
        from packages.agents.graph import route_after_schema, route_after_content_review, route_after_judge

        state: dict[str, Any] = _base_state(artifacts=[VALID_ARTIFACT], lesson_plan={"topic": "Photosynthesis"})

        schema_result = step_09_schema_validate(cast("OhMyClassState", state))
        state.update(schema_result)
        assert route_after_schema(cast("OhMyClassState", state)) == "step_10_content_review"

        content_result = step_10_content_review(cast("OhMyClassState", state))
        state.update(content_result)
        assert route_after_content_review(cast("OhMyClassState", state)) == "step_10b_llm_judge"

        judge_result = step_10b_llm_judge(cast("OhMyClassState", state))
        state.update(judge_result)
        assert route_after_judge(cast("OhMyClassState", state)) == "gate_02_content_approval"
    ...
    def test_healing_escalates_after_max_retries(self):
        from packages.agents.healing.orchestrator import HealingOrchestrator, route_after_healing

        state: dict[str, Any] = _base_state(fail_count=3, fail_type="validation", fail_layer="schema")
