...
class TestHealingOrchestrator:
    ...
    def test_replan_uses_teaching_pack_state_fields(self):
        ...
        assert result["artifact_chunks"] is None
        assert result["quality_scores"] is None
        assert result["rendered_snapshots"] is None
        ...
        assert "content_review_passed" not in result
    ...
    def test_route_after_healing_escalate(self):
        ...
        assert route_after_healing(cast("TeachingPackState", {"escalate": True})) == "escalate_node"

    def test_route_after_healing_generate(self):
        from packages.agents.healing.orchestrator import route_after_healing
        assert route_after_healing(cast("TeachingPackState", {"escalate": False})) == "step_08_generate"  # noqa: E501
        assert route_after_healing(cast("TeachingPackState", {})) == "step_08_generate"
