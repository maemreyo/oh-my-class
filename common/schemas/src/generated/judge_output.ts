...
export const LayerScoreSchema = z
	.object({
		layer: z
			.string()
			.describe(
				"Layer name: 'format_compliance', 'content_quality', or 'presentation'",
			),
		...
		weight: z
			...
			.gte(0)
			.lte(1)
			.describe("Weight for this layer in the overall score (must sum to 1.0)"),
...
export const JudgeOutputSchema = z
	.object({
		overall_score: z.number().gte(0).lte(10),
		layer_scores: z.array(LayerScoreSchema).optional(),
		critical_issues: z
			.array(z.string())
			.describe("Issues that auto-fail regardless of score")
			.optional(),
		passed: z
			.boolean()
			.describe("True if overall_score >= 7.0 and no critical issues"),
		rationale: z
			.string()
			.min(1)
			.describe("Think-before-score rationale (written before numeric scores)"),
	})
	.describe(
