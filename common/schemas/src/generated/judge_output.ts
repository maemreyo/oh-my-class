...
export const JudgeOutputSchema = z.object({ "overall_score": z.number().gte(0).lte(10), "layer_scores": z.array(LayerScoreSchema).optional(), "critical_issues": z.array(z.string()).describe("Issues that auto-fail regardless of score").optional(), "passed": z.boolean().describe("True if overall_score >= 7.0 and no critical issues"), "rationale": z.string().min(1).describe("Think-before-score rationale (written before numeric scores)") }).describe("Final judgment output from the Reviewer Agent.\n\nProduced by 3 independent judge calls; majority vote determines the final score.\nPass threshold: overall_score >= 7.0 AND no critical issues.")
