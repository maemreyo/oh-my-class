...
export const LearningObjectiveSchema = z
	.object({
		...
		bloom_level: z
			.string()
			.regex(/^(remember|understand|apply|analyze|evaluate|create)$/)
...
export const AssessmentCheckpointSchema = z
	.object({
		type: z
			.string()
			.describe(
				"Checkpoint type, e.g. 'exit_ticket', 'think_pair_share', 'quiz'",
			),
		description: z.string().min(1).max(500),
		trigger: z
			.union([z.string(), z.null()])
			.describe("When to trigger this checkpoint, e.g. 'after_phase_2'")
			.default(null),
	})
	.describe("A checkpoint within the lesson for formative assessment.");

export const LessonPlanSchema = z
	.object({
		topic: z.string().min(1).max(200),
		grade_level: z.string().describe("e.g. 'Grade 5', 'Lớp 5'"),
		subject: z.string().describe("e.g. 'math', 'english', 'science'"),
		duration_minutes: z.number().int().gte(10).lte(180),
		learning_objectives: z
			.array(LearningObjectiveSchema)
			.min(1)
			.max(10)
			.describe("Must cover ≥2 Bloom levels"),
		prerequisite_knowledge: z.array(z.string()).optional(),
		learning_plan: z
			.record(z.string(), z.any())
			.describe("Gagné 9-event phases keyed by phase name")
			.optional(),
		assessment_checkpoints: z.array(AssessmentCheckpointSchema).optional(),
	})
	.describe(
