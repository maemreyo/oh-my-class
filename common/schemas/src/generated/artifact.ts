...
export const ArtifactContentSchema = z
	.object({
		...
		theme: z.string().describe("Visual theme name").default("default"),
		title: z.string().min(3).max(200),
		sections: z
			.array(z.record(z.string(), z.any()))
			.min(1)
			.describe("List of content sections; structure varies by artifact_type"),
		metadata: z
			.record(z.string(), z.any())
			.describe("Arbitrary metadata (duration, difficulty, etc.)")
			.optional(),
		accessibility: z
			.record(z.string(), z.any())
			.describe("Language, reading_level, alt_texts, etc.")
			.optional(),
	})
	.describe(
...
export const TeachingPackSchema = z
	.object({
		run_id: z.string().describe("Pipeline run identifier"),
		artifacts: z.array(ArtifactContentSchema).optional(),
		metadata: z.record(z.string(), z.any()).optional(),
	})
	.describe("A complete teaching pack containing one or more artifacts.");
