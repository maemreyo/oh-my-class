...
export const InverseThinkingCaseSchema = z.object({ "id": z.string().min(1).max(120), "title": z.string().min(3).max(200), "alias": z.union([z.string().max(120), z.null()]).default(null), "target_concept": z.string().min(1).max(300), "foil": z.string().min(1).max(300), "disaster": z.string().min(1).max(1000), "key_clues": z.array(z.string()).min(1).max(8), "safe_zone": z.string().min(1).max(1000), "filing_note": z.string().min(1).max(1000), "student_task": z.string().min(1).max(1000), "teacher_only": z.lazy(() => InverseThinkingTeacherOnlySchema) })
...
export const InverseThinkingPackSchema = z.object({ "methodology": z.literal("inverse_thinking"), "creative_frame": z.enum(["auto","detective_case","courtroom_trial","mythbusters_lab","survival_guide","disaster_report","custom"]), "cases": z.array(z.lazy(() => InverseThinkingCaseSchema)).min(1), "summary_table": z.array(z.lazy(() => InverseThinkingSummaryRowSchema)).min(1), "student_challenges": z.array(z.lazy(() => InverseThinkingStudentChallengeSchema)).min(1), "teacher_only": z.lazy(() => InverseThinkingTeacherOnlySchema), "projection_hints": z.record(z.string(), z.array(z.string())).optional() })
