...
export const RunContractSchema = z.object({ "contract_id": z.string().min(1).max(64), "run_id": z.string().min(1).max(64), "teacher_id": z.string().min(1).max(64), "mode": z.enum(["generate_pack","diagnose_then_generate","plan_unit"]).default("generate_pack"), "topic": z.string().min(1).max(200), "grade_band": z.string().min(1).max(64), "subject": z.string().min(1).max(80), "locale": z.string().min(2).max(16), "instruction_language": z.string().min(2).max(32), "curriculum": z.union([z.string().max(80), z.null()]).default(null), "citation_locale": z.string().min(2).max(16), "artifact_types": z.array(z.enum(["lesson","worksheet","quiz","drill","recap","infographic"])).min(1), "export_formats": z.array(z.enum(["html","gift","h5p","qti","google_forms"])).min(1), "research_policy": z.enum(["basic","standard","rigorous"]).default("standard"), "config_version": z.string().min(1).max(64), "config_hash": z.string().min(64).max(64), "student_evidence": z.union([z.any(), z.null()]).default(null), "decomposition_intent": z.union([z.lazy(() => DecompositionIntentSchema), z.null()]).default(null), "revision_meta": z.lazy(() => ContractRevisionMetaSchema) })
