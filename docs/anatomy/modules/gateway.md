...
| POST | `/ops/dead-letter-jobs/{job_id}/replay` | `routers/ops.py` `replay_dead_letter_job` | #124: admin-only replay -- resets to `pending` with a clean attempt count via `TeachingPackJobStore.replay_dead_letter`; also writes an `admin.dead_letter.replayed` `RunEvent` (ADMIN visibility, `admin_id`/`job_id`/`replayed_at`) via `TeachingPackRunStore.write_event` for a who/when audit trail |
...
- **`teaching_pack_job_store.py`** — `TeachingPackJobStore` — claim/lease/mark lifecycle for `run_jobs`; `RunJobStatus.DEAD_LETTER` (#124) is an inspectable/replayable holding state (excluded from `claim_next`/`list_pending` like `FAILED`, but distinct from it) — `mark_dead_letter()` records `last_error`/`error_classification`/`dead_lettered_at`, `replay_dead_letter()` resets a dead-lettered job to `pending` with a clean attempt count. `teaching_pack_worker.py`'s `TeachingPackWorker._handle_job_error` calls `mark_dead_letter()` then `_surface_dead_letter_to_teacher()` (#124), which transitions the parent `Run` to `RunStatus.FAILED` (via `TeachingPackRunStore.transition_status`, guarded against clobbering an already-terminal run) and writes a `teaching_pack.run.dead_letter` `RunEvent` (TEACHER visibility, `message="failed - needs attention"`) — kept structurally separate from ADR-029 quality-escalate, which uses its own `GateInterrupt`/`run_escalated` notification path and is never touched by this code.
...
- **`slo_metrics.py`** + **`slo_alerting.py`** — SLO computation and alerting. `SloDimension.dead_letter_count` (#124, `slo_metrics.py`'s `_dead_letter_count_by_teacher`) counts current `RunJobStatus.DEAD_LETTER` rows per teacher/global (current backlog, not time-windowed — same pattern as `queue_depth`). `SloThresholds.max_dead_letter_count` (default `0`, env `OMC_SLO_MAX_DEAD_LETTER_COUNT`) fires a breach through the existing `evaluate_slo_breaches`/`dispatch_slo_alerts` path, tagged `SloBreach.severity="page"` (every other metric stays `"warn"`) per ADR-034 decision 2 (DLQ growth is page-level) — reuses the `job-queue-stuck.md` runbook. Note: this only adds a `severity` field to the existing single-tier webhook dispatch; full OPS-04 page-vs-warn *routing* (separate on-call channel) isn't built in this codebase yet.
...
---
_Traced from source on 2026-07-11, incrementally refreshed 2026-07-13 for #124's dead-letter teacher-surfacing/replay-audit/page-alert additions. Files examined in depth: `main.py`, `routers/` (all 17 router files), `teaching_pack_executor.py`, `teaching_pack_worker.py`, `models.py`, `teaching_pack_models.py`, `teaching_pack_quality_gate.py`, `quality_gates.py`, `quality_workflow.py`, `auth/` (all 5 files), `slo_metrics.py`, `slo_alerting.py`. 123 total files in module._
