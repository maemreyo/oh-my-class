# ADR-010: Pipeline V2 Testing and Observability

## Status

**Decided** (2026-06-27) — Pipeline V2 requires real production-path integration tests, live 9Router validation, persisted events, and Langfuse trace correlation.

## Context

The current automated tests pass, but live full-flow runs fail after teacher approval. This proves that mocked/unit-level confidence is not enough for production behavior. Pipeline V2 must validate real orchestration, persistence, web search/fetch, LLM behavior, rendering, gates, SSE, and export.
...
- full live V2 release matrix before claiming production readiness;
...
- V2 cannot be called production-ready based only on unit tests.
