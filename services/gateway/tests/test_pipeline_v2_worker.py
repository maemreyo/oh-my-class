...
from services.gateway.pipeline_v2_executor import PipelineV2ResumeJob, PipelineV2StartJob
...
from services.gateway.pipeline_v2_worker import PipelineV2Worker, PipelineV2WorkerConfig
...
class RecordingExecutor:
    start_jobs: list[PipelineV2StartJob] = field(default_factory=list)
    resume_jobs: list[PipelineV2ResumeJob] = field(default_factory=list)
    fail: bool = False
...
class TestPipelineV2Worker:
    async def test_run_one_returns_false_when_no_job(self, session: AsyncSession) -> None:
        executor = RecordingExecutor()
        worker = PipelineV2Worker(
            PipelineV2JobStore(session),
            executor,
            PipelineV2WorkerConfig(worker_id="worker-a", lease_seconds=30, idle_sleep_seconds=0),
        )
    ...
    async def test_run_one_executes_start_job_and_marks_completed(
        ...
        assert executor.start_jobs == [PipelineV2StartJob(
            run_id=run_id,
            initial_state={"run_id": run_id, "raw_request": "Teach worker"},
        )]
    ...
    async def test_run_one_executes_resume_job_and_marks_completed(
        ...
        assert did_work is True
        assert executor.resume_jobs == [PipelineV2ResumeJob(
            run_id=run_id,
    ...
    async def test_worker_promotes_eligible_queued_after_completion(
        ...
        queued_job = await PipelineV2JobStore(session).enqueue(RunJobCreate(
            job_id=f"job-{uuid4()}",
            run_id=run_id,
            kind=RunJobKind.START,
            idempotency_key=f"idem-{uuid4()}",
    ...
    async def test_cancel_run_jobs_cancels_queued_jobs(
        ...
        cancelled = await PipelineV2JobStore(session).cancel_run_jobs(run_id)
