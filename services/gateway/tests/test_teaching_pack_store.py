...
class TestTeachingPackStore:
    ...
    async def test_transition_status_persists_history(self, session: AsyncSession) -> None:
        ...
        await store.transition_status(TeachingPackStatusTransition(
            ...
            stage=TeachingPackStage.PLANNING_BLUEPRINT.value,
    ...
    async def test_events_replay_in_sequence_order(self, session: AsyncSession) -> None:
        ...
        first = await store.write_event(TeachingPackEventCreate(
            run_id=run_id,
            event_name=TeachingPackStage.SETUP_CONTRACT.started_event,
            visibility=TeachingPackEventVisibility.TEACHER,
            stage=TeachingPackStage.SETUP_CONTRACT,
        ))
        second = await store.write_event(TeachingPackEventCreate(
            run_id=run_id,
            event_name=TeachingPackStage.SETUP_CONTRACT.completed_event,
        ...
        assert [event.event_name for event in replayed] == [
            TeachingPackStage.SETUP_CONTRACT.started_event,
            TeachingPackStage.SETUP_CONTRACT.completed_event,
        ]
    ...
    async def test_concurrent_event_writes_remain_monotonic(self) -> None:
        ...
        async with session_factory() as setup_session:
            store = TeachingPackRunStore(setup_session)
            await store.create_run(TeachingPackRunCreate(
                run_id=run_id,
                teacher_id=TeacherId("teacher-a"),
        ...
        async def write_one(event_name: str) -> None:
            async with session_factory() as write_session:
                store = TeachingPackRunStore(write_session)
...
def test_teaching_pack_tables_are_registered_in_metadata() -> None:
