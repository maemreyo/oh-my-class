...
class TestPipelineV2RunRouteEdges:
    ...
    def test_resume_stale_gate_without_idempotency_key_returns_409(
        ...
        first = client.post(f"/pipeline-v2/run/{run_id}/resume", json=payload)
        second = client.post(f"/pipeline-v2/run/{run_id}/resume", json=payload)
    ...
    def test_cancel_run_cancels_run_jobs_and_emits_event(self, client: TestClient) -> None:
        run_id = RunId(f"test-{uuid4()}")
        anyio.run(_create_run_with_jobs, run_id)

        response = client.post(f"/pipeline-v2/run/{run_id}/cancel")
        stream = client.get(f"/pipeline-v2/run/{run_id}/status?replay_only=true")
        ...
        assert "event: pipeline_v2.run.cancelled" in stream.text
...
async def _create_run_with_contract_gate(run_id: RunId, gate_id: str) -> None:
    ...
    async with session_factory() as session:
        ...
        await PipelineV2ControlStore(session).create_contract(RunContractCreate(
