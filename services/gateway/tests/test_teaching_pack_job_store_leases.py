...
class TestTeachingPackJobStoreLeases:
    ...
    async def test_mark_failed_clears_running_lease(self, session: AsyncSession) -> None:
        ...
        failed = await TeachingPackJobStore(session).mark_failed(claimed.job_id)
    ...
    async def test_cancel_run_jobs_cancels_pending_and_running_jobs(
        ...
        running = await TeachingPackJobStore(session).claim_next(
            lease_owner="worker-a",
            lease_seconds=30,
            now=datetime(2026, 1, 1, tzinfo=UTC),
        )

        cancelled_count = await TeachingPackJobStore(session).cancel_run_jobs(run_id)
