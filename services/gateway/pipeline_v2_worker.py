...
if TYPE_CHECKING:
    ...
    from services.gateway.pipeline_v2_job_store import PipelineV2JobStore, RunJobRead
    from services.gateway.pipeline_v2_types import JsonObject


@dataclass(frozen=True, slots=True)
class PipelineV2WorkerConfig:
    worker_id: str
    lease_seconds: int
...
class PipelineV2JobExecutor(Protocol):
...
class PipelineV2Worker:
    def __init__(
        self,
        job_store: PipelineV2JobStore,
        executor: PipelineV2JobExecutor,
        config: PipelineV2WorkerConfig,
    ) -> None:
    ...
    async def _execute(self, job: RunJobRead) -> None:
        match job.kind:
            case RunJobKind.START:
                await self._executor.run_start_job(PipelineV2StartJob(
                    run_id=job.run_id,
                    initial_state=_initial_state(job),
                ))
            case RunJobKind.RESUME:
                await self._executor.run_resume_job(PipelineV2ResumeJob(
                    run_id=job.run_id,
