"""Teaching Pack run routes — create and resume.

The router merges sub-routers for streaming (teaching_pack_stream) and
lifecycle (teaching_pack_lifecycle) so that all teaching-packs routes are
...
from services.gateway.teaching_pack_idempotency import (
    ...
    if idempotency_key is not None:
        ...
        if existing_job is not None:
            ...
            if existing_job.payload.get("blocked_by_gate") is not None:
                return TeachingPackRunAcceptedResponse(
...
async def resume_teaching_pack_run(
    run_id: str,
    payload: TeachingPackResumeRequest,
    current_user: Annotated[User, Depends(require_teacher)],
    session: AsyncSession = TEACHING_PACK_SESSION,
    idempotency_key: Annotated[str | None, Header(alias="Idempotency-Key")] = None,
) -> TeachingPackResumeAcceptedResponse:
    ...
    try:
        ...
        if (
            validation.action is TeachingPackGateAction.EDIT
