...
This repo has traced architecture docs under `docs/anatomy/`. **Read `docs/anatomy/index.md` first** before exploring the codebase from scratch -- it has the module map, tech stack, entry points, and links to everything else below.

- `docs/anatomy/index.md` -- start here: overview, module table, entry points, codebase health signals
- `docs/anatomy/entry-points.md` -- every HTTP route / CLI command / queue consumer / cron job, in one place
- `docs/anatomy/system-diagram.md` -- module & dependency graph (Mermaid)
- `docs/anatomy/system-diagram.html` -- same diagram, interactive -- open directly in a browser
- `docs/anatomy/modules/<slug>.md` -- one file per module: responsibilities, public interface, dependencies
- `docs/anatomy/data-model.md` -- ERD-style data model
- `docs/anatomy/deployment.md` -- deployment/infra topology

Traced as of commit `63f1d9a01868`. If this looks stale against recent changes, re-run the `anatomy` skill to refresh it before trusting it -- don't re-derive the architecture from source by hand when these docs already exist.
<!-- anatomy:end -->
